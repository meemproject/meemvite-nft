...
import MeemViteArtifact from '../artifacts/contracts/MeemVite.sol/MeemVite.json'
import MeemViteUriArtifact from '../artifacts/contracts/MeemViteURI.sol/MeemViteURI.json'
import { MeemVite } from '../typechain/MeemVite'
import { MeemViteUri } from '../typechain/MeemViteUri'
...
describe('MeemVite', function Test() {
	let meemVite: MeemVite
	let meemViteUri: MeemViteUri
	let signers: SignerWithAddress[]

	beforeEach(async () => {
		signers = await ethers.getSigners()
		meemVite = (await deployContract(signers[0], MeemViteArtifact)) as MeemVite
		meemViteUri = (await deployContract(
			signers[0],
			MeemViteUriArtifact
		)) as MeemViteUri
		await meemVite.initialize('0x0000000000000000000000000000000000000000')
	...
	it('Can set URI contract as owner', async () => {
		const { status } = await (
			await meemVite
				.connect(signers[0])
				.setTokenURIContractAddress(meemViteUri.address)
		).wait()

		assert.equal(status, 1)
	})

	it('Can not set URI contract as non-owner', async () => {
		await assert.isRejected(
			meemVite
				.connect(signers[1])
				.setTokenURIContractAddress(meemViteUri.address)
